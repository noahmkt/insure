 ...
 * 검색결과 노출용 짧은 설명 생성.
 *
 * 네이버 웹마스터 가이드 권장치는 description 80자 이내이고 그보다 길면 잘려 나온다.
 * 반면 SNS 공유 카드(og:description)는 더 긴 설명이 유리하므로 두 값을 분리한다.
 * 네이버는 description 과 og:description 이 다르면 색인 시 더 적합한 값을 선택한다.
 *
 * 자르는 순서: 첫 문장 → 문장 경계 → 어절 경계. 단어 중간에서 끊지 않는다.
 */
...
export function metaTags({
  ...
  // 검색결과용은 80자 이내로, 공유 카드용은 전문으로 내보낸다
  const metaDesc = shortDescription(description);
  const tags = [
...
export function organizationSchema() {
  ...
  // 네이버 '사이트 연관채널' — 운영 중인 공식 채널을 sameAs 로 연결하면
  // 검색결과에서 사이트와 채널이 하나의 출처로 묶인다.
